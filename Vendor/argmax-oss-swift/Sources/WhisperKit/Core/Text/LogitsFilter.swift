...
//       }
    ...
    private func sumOfProbabilityOverTimestampsIsAboveAnyOtherToken(logits: MLMultiArray, timeTokenBegin: Int) -> Bool {
        ...
        return sumOfProbabilityOverTimestampsIsAboveAnyOtherTokenCPU(logits: logits, timeTokenBegin: timeTokenBegin)
#else
...
#if (os(macOS) || targetEnvironment(macCatalyst)) && arch(x86_64)
    private func sumOfProbabilityOverTimestampsIsAboveAnyOtherTokenCPU(logits: MLMultiArray, timeTokenBegin: Int) -> Bool {
        let timeTokenBeginOffset = logits.linearOffset(for: [0, 0, timeTokenBegin])
        guard timeTokenBeginOffset > 0, timeTokenBeginOffset < logits.count else {
            return false
        }

        var maxTextTokenLogit = -Float.infinity
        for index in 0..<timeTokenBeginOffset {
            maxTextTokenLogit = max(maxTextTokenLogit, logits[index].floatValue)
        }

        var maxTimestampLogit = -Float.infinity
        for index in timeTokenBeginOffset..<logits.count {
            maxTimestampLogit = max(maxTimestampLogit, logits[index].floatValue)
        }

        guard maxTimestampLogit.isFinite else {
            return false
        }

        var timestampExpSum: Float = 0
        for index in timeTokenBeginOffset..<logits.count {
            timestampExpSum += expf(logits[index].floatValue - maxTimestampLogit)
        }

        guard timestampExpSum.isFinite, timestampExpSum > 0 else {
            return false
        }

        let timestampLogSumExp = maxTimestampLogit + logf(timestampExpSum)
        return timestampLogSumExp > maxTextTokenLogit
    }
#endif
}
